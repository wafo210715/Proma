...
export function closeCanvasInSplit(store: JotaiStore): void {
  ...
  const browserTab: TabItem = tabs.find((t) => t.id === BROWSER_ID) ?? {
    id: BROWSER_ID, type: 'browser', sessionId: BROWSER_ID, title: BROWSER_TITLE,
  }
  store.set(tabsAtom, [scratchTab, canvasTab, browserTab, ...nonPinnedTabs])
}
